# -*- coding: utf-8 -*-
"""
WEEK_9_APP.PY - Complete Graphical Interface for Postural Analysis
...
class PostureAnalysisApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Sistema de Análisis Postural - Semana 9")
    ...
    def _download_video(self):
        ...
        try:
            shutil.copy(
                video_path,
                os.path.join(save_dir, "output_alerts.mp4")
            )
            messagebox.showinfo("Éxito", "Video descargado")
            self.status_var.set("Video descargado correctamente")
        except Exception as e:
            messagebox.showerror("Error", f"Error al descargar: {str(e)}")
    
    def _view_video(self):
        """Opens the analysis video."""
        if not self.current_results:
            messagebox.showwarning("Error", "Primero realiza un análisis")
            return
        ...
        if os.path.exists(video_path):
            try:
                subprocess.Popen(['xdg-open', video_path])  # Linux default
            except:
                try:
                    subprocess.Popen(['vlc', video_path])  # Try VLC directly
                except:
                    try:
                        ...
                        try:
                            ...
                            messagebox.showwarning("Info", f"Video disponible en:\n{video_path}")
        else:
            messagebox.showwarning("Error", "Video no encontrado")
