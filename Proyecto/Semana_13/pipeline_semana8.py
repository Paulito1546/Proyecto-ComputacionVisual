...
def compute_all_angles_from_keypoints(keypoints):
    ...
    try:
        ...
        angles['tobillo_izquierdo'] = calc_angle(kp[25], kp[27], kp[31] if len(kp) > 31 else kp[27])
        angles['tobillo_derecho'] = calc_angle(kp[26], kp[28], kp[32] if len(kp) > 32 else kp[28])
        ...
        angles['muneca_izquierda'] = calc_angle(kp[13], kp[15], kp[19] if len(kp) > 19 else kp[15])
        angles['muneca_derecha'] = calc_angle(kp[14], kp[16], kp[20] if len(kp) > 20 else kp[16])
        ...
        for name in ['rodilla_izquierda', 'rodilla_derecha', 'tobillo_izquierdo', 'tobillo_derecho',
                     'cadera_izquierda', 'cadera_derecha', 'codo_izquierdo', 'codo_derecho',
                     'muneca_izquierda', 'muneca_derecha', 'hombro_izquierdo', 'hombro_derecho']:
...
def draw_alert_panel(frame, frame_alerts, width, height):
    ...
    if not frame_alerts:
        ...
        overlay = frame.copy()
        cv2.rectangle(overlay, (margin, y_pos), (margin + panel_w, y_pos + panel_h), good_color, -1)
        frame = cv2.addWeighted(overlay, 0.85, frame, 0.15, 0)
        
        cv2.rectangle(frame, (margin, y_pos), (margin + panel_w, y_pos + panel_h), good_color, 3)
        
        cv2.putText(frame, "POSTURA BUENA",
                    (margin + 20, y_pos + 30),
    ...
    for i, alert in enumerate(frame_alerts[:max_alerts]):
        ...
        overlay = frame.copy()
        cv2.rectangle(overlay, (margin, y_pos), (margin + panel_w, y_pos + item_height), bg_color, -1)
        frame = cv2.addWeighted(overlay, 0.85, frame, 0.15, 0)
        
        cv2.rectangle(frame, (margin, y_pos), (margin + panel_w, y_pos + item_height), bg_color, 3)
        
        cv2.putText(frame, f"{sev}: {art}",
                    (margin + 15, y_pos + 25),
...
class AsimetriaAnalyzer:
    ...
    def analizar_articulacion(self, nombre: str, derecha: np.array, izquierda: np.array) -> Dict:
        ...
        indice_simetria = 100 * (2 * diferencias) / (np.abs(derecha) + np.abs(izquierda) + 1e-6)
        ...
        if nombre in self.umbrales_por_articulacion:
            ...
            umbrales = {'diferencia': self.umbral_diferencia, 'simetria': self.umbral_simetria, 'correlacion': self.umbral_correlacion}
        
        if diferencia_media > umbrales['diferencia']:
            severidad = 'ALTA' if diferencia_media > umbrales['diferencia'] * 2 else 'MEDIA'
        ...
        if indice_simetria_medio > umbrales['simetria']:
            severidad = 'ALTA' if indice_simetria_medio > umbrales['simetria'] * 2 else 'MEDIA'
            alertas_frame.append({
                'severidad': severidad,
                'tipo': 'SIM_INDICE',
    ...
    def generar_tabla_resumen(self) -> pd.DataFrame:
        ...
        for nombre, metricas in self.metricas.items():
            if nombre in self.umbrales_por_articulacion:
                ...
                umbrales = {'diferencia': self.umbral_diferencia, 'simetria': self.umbral_simetria}
            data.append({
...
def run_pipeline(input_path: str, tpose_path: str = None, output_base_dir: str = ".",
                 window_size: int = WINDOW_SIZE, analyze_every: int = ANALYZE_EVERY,
    ...
    try:
        while True:
            ...
            if results.pose_landmarks:
                ...
                # Análisis periódico
                if frame_idx % analyze_every == 0 and len(angle_tracker.to_dataframe()) >= 2:
                    ...
                    for a in frame_alerts:
                        ...
                        if nombre == 'rodilla': current_alert_joints.update([25, 26])
                        elif nombre == 'tobillo': current_alert_joints.update([27, 28])
                        elif nombre == 'cadera': current_alert_joints.update([23, 24])
                        elif nombre == 'codo': current_alert_joints.update([13, 14])
                        elif nombre == 'muneca': current_alert_joints.update([15, 16])
                        elif nombre == 'hombro': current_alert_joints.update([11, 12])
                
                draw_skeleton(frame, keypoints, frame.shape, alert_joints=current_alert_joints)
    ...
    if alerts_per_frame:
        ...
        df_alerts = pd.DataFrame(columns=['frame', 'articulacion', 'severidad', 'tipo', 'mensaje'])
        df_alerts.to_csv(alerts_csv_path, index=False)
