...
function Scene({ timeOfDay, cameraType, showDepth, command, eegValue }) {
  ...
  // Effect for managing illumination change from voice comamnds 
  useEffect(() => {
    const light = scene.getObjectByName("MainLight");
    if (light) mainLight.current = light;
  }, [scene]);
  ...
  useEffect(() => {
    ...
    if (command === 'day flash' && mainLight.current) {
      const prev = mainLight.current.intensity;
      mainLight.current.intensity = prev + 9;
      setTimeout(() => (mainLight.current.intensity = prev), 800);
    }

    // "flor" command → make plants "grow"
    if ((command === 'flower' || command == 'flor') && plantsRef.current) {
      plantsRef.current.traverse((obj) => {
        if (obj.isMesh) {

          // Saca el bounding box de la flor
          const box = new THREE.Box3().setFromObject(obj);
          const height = box.max.y - box.min.y;

          // Guarda escala y posición anteriores
          const prevScale = obj.scale.clone();
          const prevPos = obj.position.clone();

          // Nueva escala incremental
          const newScale = prevScale.clone().multiplyScalar(1.1);
          obj.scale.copy(newScale);

          // Compensar la subida usando bounding box
          const deltaHeight = (newScale.y - prevScale.y) * height;
          obj.position.y = prevPos.y - deltaHeight * 0.5;
        }
      });
    }

    // "noche" command → dim the light
    if (command === 'night flash' && mainLight.current) {
      const prev = mainLight.current.intensity;
      mainLight.current.intensity = 0;
      setTimeout(() => (mainLight.current.intensity = prev), 1000);
    }
  }, [command]);
